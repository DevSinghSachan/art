...
def get_args():
    ...
    group = parser.add_argument_group(title='save ART retriever')
    group.add_argument('--load', type=str, required=True, help='ART model path')
    group.add_argument('--save', type=str, required=True, help='path to save model')
    group.add_argument('--submodel-name', type=str, required=True, choices=['retriever'],
                       help='sub-model of ART to save')
